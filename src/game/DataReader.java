...
public class DataReader {
	...
	public ArrayList<Level> getLevelData(String mainFile) throws IOException {
		...
		pw.println();
		pw.println("Done reading all data. Took " + (System.currentTimeMillis() - startTime) + " ms.");
		pw.close();
		return levels;
	}
	...
	public int[] getSettings() throws IOException {
		...
		try {
			...
			System.out.println("Error in loading settings.txt, default settings loaded");
	...
	private ArrayList<Level> readFile(File file) throws IOException {
		...
		try {
			while(infile.hasNext()) {
				currentLineN++;
				String line = infile.nextLine().toLowerCase().trim().replaceAll(" ", "");
				...
				if(line.length() > 2 && !line.substring(0,2).equals("//")) {
					...
					if(line.substring(0, 4).equals("ball")) {
						...
						Color c = CalcHelp.readColor(data[3]);		
						Body tempBod;
						if(data.length > 4) {
							tempBod = new Body(x, y, r, c, Integer.parseInt(data[4]));
						} else {
					...
					// Read planet data in
						...
						Color c = CalcHelp.readColor(data[3]);		
						if(data.length > 4) {
							bod.add(new Body(x, y, r, c, Integer.parseInt(data[4])));
						} else {
					...
					// read moon data in - attach to previous body
						...
						int r = Integer.parseInt(data[2]);
							// radius of ball and moon are added to the distance (moon parameter is the distance from center to center)
						int d = Integer.parseInt(data[1]) + bod.get((bod.size() - 1 )).getRadius() + r;
						Color c = CalcHelp.readColor(data[3]);
						Moon m = new Moon(a, d, r, c, bod.get(bod.size() - 1));
						bod.get((bod.size() - 1 )).addMoon(m); // attaches moon to last body in bod
					}
					// read warp data in
					...
					// read top-left input blockage data in
						...
						int cX = leftX + sX;
						int cY = topY  + sY;
						Color c = CalcHelp.readColor(data[4]);
					...
					// read final level data in
						...
						levels.add( new Level(b, bod, warppts, gs, bs, followF, g) );
						...
						pw.println("Failed to read" + file + " - Invalid type at line " + currentLineN + ".");
						pw.close();
						JOptionPane.showMessageDialog(null, "GravityGolf is unable to load " + file + ".", "Error", JOptionPane.ERROR_MESSAGE);
						errorFound = true;
					}
				}
			}
		} catch(Exception e) {
			pw.println("Failed to read " + file + " - Invalid entry at line " + currentLineN + ".");
			pw.println(e);
			System.out.println("Failed to read " + file + " - Invalid entry at line " + currentLineN + ".");
			System.out.println(e);
			e.printStackTrace();
			pw.close();
			//System.exit(0);
			if(!errorFound) JOptionPane.showMessageDialog(null, "GravityGolf is unable to load " + file + " at line " + currentLineN + ".", "Error", JOptionPane.ERROR_MESSAGE);
			errorFound = true;
			infile.close();
			return null;
		}
		pw.println("Done reading " + file + ". Took " + (System.currentTimeMillis() - t) + " ms.");
		infile.close();
		return levels;
