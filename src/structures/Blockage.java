...
public class Blockage {
	...
	public Blockage(java.awt.Point p1, java.awt.Point p2, Color c) {
		drawXSize = Math.abs(p1.x - p2.x); xSize = drawXSize / 2;
		drawYSize = Math.abs(p1.y - p2.y); ySize = drawYSize / 2;
		centerX = (p1.x + p2.x) / 2;	
		centerY = (p1.y + p2.y) / 2;
		center = new Point2d(centerX, centerY);
		color = c;
	...
	public void draw(int dx, int dy, Graphics2D g) {
		...
		g.setPaint(new GradientPaint(drawX + dx, drawY + dy, color, drawX + drawXSize, drawY + drawYSize, CalcHelp.getShiftedColor(color, 40)));			
	...
	public String toString() {	
		return "rect(" + centerX + ", " + centerY + ", " + xSize + ", " + ySize + ", " + CalcHelp.getColorDisplay(color) + ")";
	}	
