...
public final class CalcHelp {
	...
	public static int gaussianInteger(double mean, double standardDeviation) {
		return (int) Math.round(randomGenerator.nextGaussian() * standardDeviation + mean);
	}
	...
	public static double getAcceleration(Point2d p1, Particle p, double g) {
		return g *  p.getRadius()  / getDistanceSquared(p1, p.getCenter());  // G m1 m2 / d^2, m1 = 1, m2 = radius
	}

	public static double getAcceleration(Point2d p, Body b, double g) {
		return g *  b.getMass()  / getDistanceSquared(p, b.getCenter());  // G m1 m2 / d^2, m1 = 1, m2 = radius
		/* Note: mass1 is ignored for the sake of calculation
		 * Because: F_net = m*a  -> a = F_net / m (F_net = the sum of graviational forces = F_g)
		 *  F_g   = (G*m1*m2 / d^2)
		 *	F_net = (G*m1*m2 / d^2)
		 *  a     = (G*m1*m2 / d^2) / m1
		 *	a     = (G*m2    / d^2)
		 *	  Thus: the mass of the ball has no effect of the acceleration due to gravity upon it - thanks galileo!
		 */
	}
	...
	public static String getColorDisplay(Color c) {
		...
		if(c.equals(new Color(128, 0, 128))) return "purple";
		if(c.equals(new Color(127, 0, 255))) return "violet";
	...
	public static Color readColor(String str) {
		...
		if(str.equals("purple"))  return new Color(128, 0, 128);
		if(str.equals("violet"))  return new Color(127, 0, 255);
