...
object CalcHelp {
  ...
  def getAcceleration(location: Point2d, sourceLocation: Point2d,
                      strength: Double, g: Double): Double = g * strength / location.distanceSquared(sourceLocation)
  ...
  /** Runs a function across all non-null elements on an ArrayList. */
  def map[A](f: A => Unit, list: java.util.List[A]): Unit = {
    if (list != null)
